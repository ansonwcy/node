...
import * as hashDir_1 from '../src/hashDir'
...
describe('IPFS', function () {
  ...
  it('hash null content V0', async () => {
    let result = await hashContent('', 0);
    let expected_result = {
      cid: 'QmbFMke1KXqnYyBBWxB74N4c5SBnJMVAiMNRcGu6x1AwQH',
      size: 6
    }
    assert.strictEqual(eval(result).cid, expected_result.cid);
  });
  it('hash null content V1', async () => {
    let result = await hashContent('', 1);
    let expected_result = {
      cid: 'bafkreihdwdcefgh4dqkjv67uzcmw7ojee6xedzdetojuzjevtenxquvyku',
      size: 6
    }
