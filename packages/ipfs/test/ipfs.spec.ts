...
// describe('IPFS (Non zero dependency)', function () { //works
//     it('hash dir', async () => {
//         let path = Path.join(__dirname, './dir');
//         console.log('path', path)
//         let result = await Hash.ofDir(path); //match with web3.storage
//         let result_str = result?.toString()
//         assert.deepStrictEqual(result_str, {
...
describe('IPFS (Non zero dependency)', function () { //works
    it('hash dir', async () => {
        let path = Path.join(__dirname, './dir');
        console.log('path', path)
        let result = await hashDir(path); //match with web3.storage
